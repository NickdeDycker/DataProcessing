...
def scrape_movie_page(dom):
    ...
    overview = dom.by_tag('td#overview-top')[0]  # Get the 'main div' with all the information of the movie.
    title = overview.by_tag('span.itemprop')[0].content
    year = overview.by_tag('span.nobr')[0]('a')[0].content

    infobar = overview.by_tag('div.infobar')[0]  # Select the small bar with duration and genre.
    duration = overview.by_attr(itemprop='duration')[0].content.split()[0]

    genres = ''
    for genre in infobar.by_tag('span.itemprop'):
    ...
    # Get the actors in a string with a semicolon separating
    actors = ''
    for actor in overview.by_tag('div.txt-block')[2]('a')[:-1]:
        actor = actor.by_tag('span.itemprop')[0].content
        actors += actor + ';'
    actors = actors[0:-1]

    star_box = overview.by_tag('div.star-box-details')[0]  # The HTML block with ratings in it.
    rating = star_box.by_attr(itemprop='ratingValue')[0].content
    n_ratings = star_box('a')[0].by_attr(itemprop='ratingCount')[0].content
    n_ratings = n_ratings.replace(',', '')  # Removing the comma's
